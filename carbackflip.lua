...
local function isUpsideDown(cframe)
	local upVector = cframe.YVector
	return upVector.Y < -0.5
end
...
local function makeCarJump()
	...
	local wasUpsideDown = isUpsideDown(collisionPart.CFrame)
		...
		connection = RunService.Heartbeat:Connect(function()
			...
			if backflipMode then
				...
				if shouldComplete or elapsed >= jumpDuration then
					...
					if jumpData.wasUpsideDown then
						rotationAmount = easedProgress * 180
					else
						rotationAmount = easedProgress * 360
					end
